//context for authentication state and profile data its used in App.tsx for 
// wrapping the app with AuthProvider and storing auth state
...
type AuthValue = {
  ...
  signInWithOAuth: (provider: 'apple' | 'facebook') => Promise<void>;
...
export const AuthProvider: React.FC<React.PropsWithChildren> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (session?.user) {
        ...
        // Cache profile locally in SQLite for offline use
        try {
  ...
  // Allows login using email or username. When an identifier does not look like an email,
  // it will look up the user's email by username in the `profiles` table (requires `email` column).
  const signInWithIdentifier = async (identifier: string, password: string) => {
    const trimmed = identifier.trim();
    if (trimmed.includes('@')) {
      return signInWithEmail(trimmed, password);
    }
    // Treat as username; fetch email from profiles
    const { data, error } = await supabase
      .from('profiles')
      .select('email')
      .eq('username', trimmed)
  ...
  const signUpWithEmail = async (email: string, password: string, username?: string) => {
    ...
    if (error) throw error;
    // If email confirmation is required, the session will be null until the user confirms.
    // If we have a session immediately, try to set the username in the profiles table.
  ...
  const signInWithOAuth = async (provider: 'apple' | 'facebook') => {
    const redirectTo = process.env.EXPO_PUBLIC_SUPABASE_REDIRECT_URL;
