...
export const Login: React.FC = () => {
  ...
  const onOAuth = async (provider: 'apple' | 'facebook') => {
    try {
      setBusy(true);
      ...
      <View style={styles.container}>
        ...
        <View style={styles.socialRow}>
          <Pressable
            ...
            style={({ pressed }) => [styles.socialBtn, styles.socialBtnLeft, { opacity: pressed || busy ? 0.9 : 1 }]}
          >
            <View style={styles.socialIconWrap}>
              <Ionicons name="logo-apple" size={20} color={palette.text} />
            </View>
            <Text style={{ color: palette.text, fontWeight: '600' }}>Apple</Text>
          </Pressable>
          <Pressable
            onPress={() => onOAuth('facebook')}
            style={({ pressed }) => [styles.socialBtn, styles.socialBtnRight, { opacity: pressed || busy ? 0.9 : 1 }]}
          >
            <View style={styles.socialIconWrap}>
              <Ionicons name="logo-facebook" size={20} color={palette.text} />
            </View>
            <Text style={{ color: palette.text, fontWeight: '600' }}>Facebook</Text>
          </Pressable>
        </View>
        {/* Phone login removed per request */}
